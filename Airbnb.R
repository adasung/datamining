...
numericData <- cleanData %>% mutate_if(is.factor, as.numeric)
...
library(Matrix)
# Build model using gradient boosting
numericDataMatrix <- sparse.model.matrix(price~.-1, data=numericData)
#numericDataMatrix <- sparse.model.matrix(log(price)~.-1, data=numericData)

label_matrix <- numericData[,"price"]
sparse_matrix <- xgb.DMatrix(data=as.matrix(numericDataMatrix), label=label_matrix)
xgboost_model <- xgboost(data = sparse_matrix, label = label_matrix, max_depth = 10,
                         eta = 1, nthread = 2, nrounds = 100,objective = "reg:squarederror")
...
# Make predictions & calculate MAPE with the gradient boosting model
xgboost_pred <- predict(xgboost_model, sparse_matrix) 
xgboost_mape <- mean(abs((xgboost_pred - numericData$price))/xgboost_pred)
#xgboost_mape <- mean(abs((xgboost_pred - log(numericData$price))/xgboost_pred))
xgboost_mape # MAPE=0.1381223

# Most important Features
# host_id>longitude>latitude>room_type
...
?buildExplainer
explainer = buildExplainer(xgboost_model,sparse_matrix, type="reg:linear")
pred.breakdown = explainPredictions(xgboost_model, explainer, sparse_matrix)

# Explain a single data row
showWaterfall(xgboost_model, explainer, sparse_matrix, numericDataMatrix,  2, type = "reg:linear")
showWaterfall(xgboost_model, explainer, sparse_matrix, numericDataMatrix,  45379, type = "reg:linear")
...
hostids <- cleanData %>%
  group_by(neighbourhood_group, host_id)%>%
  summarize(Mean_price = mean(price), min=min(price), max=max(price))%>%
  arrange(desc(host_id)) 
filter(hostids, between(row_number(), 1, 10) )

cleanData %>%
  filter(host_id>27430000 ) %>%
  summarize(Mean_price = mean(price), min=min(price), max=max(price))
...
hostids <- cleanData %>%
  ...
  arrange(desc(host_id)) 
filter(hostids, between(row_number(), 1, 10) )
plot(cleanData$host_id, cleanData$price)


# Price vs. longitude
plot(myData$longitude, myData$price)
options(pillar.sigfig=4)
cleanData %>% group_by(neighbourhood_group) %>% summarise(Mean_Logitude = mean(longitude), Mean_Price = mean(price),
                                                          Min = min(price),                  
                                                          Q1 = quantile(price, 0.25), 
                                                          Q3 = quantile(price, 0.75),
                                                          Max = max(price),
                                                          SD = sd(price), count=n()) %>% arrange(Mean_Price)


# Price by neighborhood_group and room_type
cleanData %>% group_by(neighbourhood_group) %>% summarise(Mean = mean(price), Median = median(price),
                                                          Min = min(price),                  
                                                          Q1 = quantile(price, 0.25), 
                                                          Q3 = quantile(price, 0.75),
                                                          Max = max(price),
...
# availability_365
mean(cleanData$availability_365)
cleanData %>% group_by(neighbourhood_group, room_type) %>% summarise(Mean_availability= mean(availability_365), Mean = mean(price), Median = median(price),
                                                                     Min = min(price),                  
                                                                     Q1 = quantile(price, 0.25), 
                                                                     Q3 = quantile(price, 0.75),
                                                                     Max = max(price),
...
plot(myData$availability_365, myData$price)
